...
import { BlueprintRecord, idbAtom } from './idbAtom'
  ...
  atom(
    ...
    async (get, set, force = false) => {
      ...
      if (data.nonFungible) {
        ...
        const mintInfo = program && await getMint(
        ...
        const metadataPointer = getMetadataPointerState(mintInfo as Mint)

        const metadata = program && await getTokenMetadata(
          program.provider.connection,
